...
(table) => ({
    ...
    ckAverageRating: check('average_rating', sql`average_rating >= 0 AND average_rating <= 5`),
})
);
...
export const bookings = pgTable(
  'bookings',
  {
    ...
    sessionPrice: decimal('session_price', { precision: 10, scale: 2 }).notNull(), // Price at time of booking
...
export const transactions = pgTable(
  ...
  (table) => ({
    // FIXED: Wrapped check strings in sql``
    ckStatus: check('status', sql`status IN ('pending', 'successful', 'failed')`),
